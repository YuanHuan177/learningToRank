...
X = joblib.load(r"../data/reformat_2014.jl").drop(columns=['nsrdzdah', 'WTBZ'])
Y = joblib.load(r"../data/reformat_2014.jl")['WTBZ']
...
joblib.dump(som.get_weights(), '../data/som20*20.jl')

som = MiniSom(20, 20, dn, sigma=1.0, learning_rate=1)
som_weights = joblib.load('../data/som20*20.jl')
dism = som.distance_map()
t = X.values[Y.values == 1, :]
mapcount2 = som.activation_response(t)
mapcount = som.activation_response(X.values)

pd.DataFrame(mapcount).to_csv('../data/allcount_before.csv')  # 类内样本数量
pd.DataFrame(mapcount2 / mapcount).to_csv('../data/err_before.csv')  # 分类标签分歧
pd.DataFrame(dism).to_csv('../data/dism_before.csv')  # 类间距离
allcount = pd.read_csv('../data/allcount_before.csv', header=0, index_col=0).values
err = pd.read_csv('../data/err_before.csv', header=0, index_col=0).values
dism = pd.read_csv('../data/dism_before.csv', header=0, index_col=0).values
...
for i in range(mr):
    for j in range(mc):
        if allcount[i, j] == 0 or (allcount[i, j] < 40 and dism[i, j] > 0.2 and err[i, j] < 0.1):
            mask[i, j] = 0
mask = mask.astype('int8')
XY_mask = [i for i, x in enumerate(X.values) if mask[som.winner(x)] == 1]  # 保留下来数据的下标行数
joblib.dump(X.values[XY_mask], '../data/filtered_inputFeatures.jl')
joblib.dump(Y.values[XY_mask], '../data/filtered_categoryLabel.jl')
joblib.dump(nsr.values[XY_mask],'../data/filtered_nsr.jl')
...
for (i, j) in coreindex.keys():
    ...
    core /= count
    cores[(i, j)] = core
joblib.dump(cores, '../data/kmeans-init.jl')

X = joblib.load('../data/filtered_inputFeatures.jl')
Y = joblib.load('../data/filtered_categoryLabel.jl')
cores = joblib.load('../data/kmeans-init.jl')
filter_nsr = joblib.load('../data/filtered_nsr.jl')
k = len(cores)
dm, dn = X.shape
...
joblib.dump(clean_reformat_2014, '../data/clean_reformat_2014.jl')
