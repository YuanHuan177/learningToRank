...
m,n =joblib.load(r"../data/reformat_2014.jl").shape
...
def loadDat():
    ...
    finaltrain=pd.concat([train,test]).sample(5911)
    print len(test[test['wtbz']==1]),len(test[test['wtbz']==0])
    print len(train[train['wtbz'] == 1]), len(train[train['wtbz'] == 0])
    # d = pd.concat([train[train['wtbz']==0],train[train['wtbz']==1].head(1650)]).reset_index(drop=True)
    #
    # print d
    # print train[train['wtbz']==0].shape
    return finaltrain, test  # 注意这里时 test 为评价指标  all_data 则为给每个节点得分
...
myvars = ['dtDefaultPred', 'mlpDefaultPred','knnDefaultPred']
seed = 5
t = 'ensemble'
kf = KFold(5, shuffle=True, random_state=seed)
train[t + 'DefaultPred'] = 0.0
...
# compliance.fillna(-1, inplace=True)
# compliance.to_csv(r"../data/compliance" , encoding='utf-8', index=False)
...
methods = [ 'lda', 'lr', 'svc','bayes','rf','gbm','ensemble']
n = len(methods)
# 对比方法指标
for i in range(n):
    print '========' + str(methods[i])
