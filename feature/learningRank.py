...
def initSplitData(all_data):
    P_Test = all_data[all_data["WTBZ"] == 1.0].sample(1000)
    num = len(P_Test)
    N_Test = all_data[all_data["WTBZ"] == 0].sample(n=num)
    train_data = pd.concat([P_Test, N_Test])
...
def saveScore(score, test_data, path):
...
print train_label_data2.shape

model = LogisticRegression(random_state=1)
model.fit(train_feature_data, train_label_data2)
test_data2, test_feature_data2 = splitTestData(all_data)
score = pd.DataFrame(model.predict(test_feature_data2), columns=["SCORE"]).set_index(test_feature_data2.index)
saveScore(score, test_data2, r"../data/compliance1")
