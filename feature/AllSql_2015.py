...
table_list_2015 = {
    'ZCDZ_YB':  # 注册地址邮编
        ...
        FROM DJ_NSRXX_KZ a RIGHT JOIN (SELECT NSRDZDAH FROM LG_DATA14_15 WHERE YEARS=2015) b ON a.NSRDZDAH = b.nsrdzdah
        ''',
    'LSGX':  # 隶属关系
        '''
       SELECT
        ...
        FROM DJ_NSRXX a RIGHT JOIN (SELECT NSRDZDAH FROM LG_DATA14_15 WHERE YEARS=2015) b ON a.NSRDZDAH = b.nsrdzdah
        ''',
    'HY':  # 行业
        '''
       SELECT
    ...
    'ZCDNSR_NUM':  # 注册地址的注册纳税人个数
        ...
        FROM
          (SELECT
           ...
           WHERE a.ZCDZ = b.ZCDZ)
          p RIGHT JOIN (SELECT NSRDZDAH FROM LG_DATA14_15 WHERE YEARS=2015) q ON p.NSRDZDAH = q.nsrdzdah
        ''',
    'ZRR_NUM':  # 关联自然人(投资方、股东)个数
        ...
        RIGHT JOIN (SELECT NSRDZDAH FROM LG_DATA14_15 WHERE YEARS=2015) b ON a.NSRDZDAH = b.nsrdzdah       
        ''',
    'FDDBR_AGE':  # 法定代表人年龄
        '''
        SELECT
    ...
    'FDDBR_JG':  # 法定代表人籍贯
        ...
        FROM (Select * from DJ_NSRXX WHERE REGEXP_LIKE(ZJHM, '^[0-9]{2}.*$')) a RIGHT JOIN (SELECT NSRDZDAH FROM LG_DATA14_15 WHERE YEARS=2015) b ON a.NSRDZDAH = b.nsrdzdah
        ''',
    'CWFZR_AGE':  # 财务负责人年龄
        '''
        SELECT
    ...
    'CWFZR_REGION':  # 财务负责人所属地区
        ...
        FROM (Select * from DJ_NSRXX_KZ WHERE REGEXP_LIKE(CWFZR_ZJHM, '^[0-9]{2}.*$')) a RIGHT JOIN (SELECT NSRDZDAH FROM LG_DATA14_15 WHERE YEARS=2015) b ON a.NSRDZDAH = b.nsrdzdah
        ''',
    'BSR_AGE':  # 办税人年龄
        '''
        SELECT
    ...
    'BSR_REGION':  # 办税人所属地区
        ...
        FROM (Select * from DJ_NSRXX_KZ WHERE REGEXP_LIKE(BSR_ZJHM, '^[0-9]{2}.*$')) a RIGHT JOIN (SELECT NSRDZDAH FROM LG_DATA14_15 WHERE YEARS=2015) b ON a.NSRDZDAH = b.nsrdzdah
        ''',
    'CYRS':  # 从业人数
        '''
        SELECT
    ...
    'TZZE':  # 投资总额
        ...
        FROM (SELECT
              ...
              FROM DJ_TZF a
              WHERE EXTRACT (YEAR FROM a.LRRQ) <2016
              GROUP BY NSRDZDAH) a RIGHT JOIN (SELECT NSRDZDAH FROM LG_DATA14_15 WHERE YEARS=2015) b ON a.NSRDZDAH = b.nsrdzdah
        ''',
    'ZCZB':  # 注册资本
        '''
        SELECT
    ...
    'SF_RATIO_AVG':  # 年度月份  税负平均变动率
        ...
        RIGHT JOIN (SELECT NSRDZDAH FROM LG_DATA14_15 WHERE YEARS=2015) q
    ...
    'IS_JC':  # 是否被稽查
        ...
        (SELECT
            ...
            GROUP BY
                NSRDZDAH) a  RIGHT JOIN (SELECT NSRDZDAH FROM LG_DATA14_15 WHERE YEARS=2015) b ON a.NSRDZDAH = b.nsrdzdah
        ''',
    'AJLY':  # 案件来源类型
        '''
       SELECT b.nsrdzdah,
        ...
        RIGHT JOIN (SELECT NSRDZDAH FROM LG_DATA14_15 WHERE YEARS=2015) b
    ...
    'WTBZ':  # 企业标签
        '''
        SELECT nsrdzdah,(case when WTBZ = 'Y' then 1 else 0 end) as WTBZ from LG_DATA14_15 WHERE YEARS=2015
        '''
}
