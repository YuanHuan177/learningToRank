...
import numpy as np
import six #six是用来兼容python 2 和 3
import pickle
import scipy
import chainer
...
class NN(object):
    ...
    def predictTargets(self, x_pred, batchsize):
        N_pred = len(x_pred)
        y_pred = np.zeros(0)
        for j in tqdm(six.moves.range(0, N_pred, batchsize)):  #封装一个进度条，处理那些在2 和 3里面函数的位置有变化，使用move 可屏蔽变化
            x = chainer.Variable(np.asarray(x_pred[j:j + batchsize]), volatile='on')  #Variable 类是把numpy.ndarray数组包装在内的计算模块
