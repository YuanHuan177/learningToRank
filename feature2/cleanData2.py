...
DingXingIndex.fillna(DingXingIndex.mode().T[0], inplace=True)
...
mutualIndexXing = SelectKBest(mutual_info_classif, k=18).fit_transform(oneHotXingIndex, label)
select2 =SelectKBest(mutual_info_classif, k=18).fit(oneHotXingIndex, label).get_support(indices=True)
...
select3 =SelectKBest(mutual_info_classif, k=18).fit(StandardIndex, label).get_support(indices=True)
