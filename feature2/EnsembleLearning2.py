...
def runRandomUnderSample(train, test, seed):
    ...
    mlpDefault = baseModel(MLPClassifier(), vars, "mlp", train, test, seed)
    ldaDefault = baseModel(LinearDiscriminantAnalysis(), vars, "lda", train, test, seed)
    lrDefault = baseModel(LogisticRegression(), vars, "lr", train, test, seed)
    dtDefault = baseModel(DecisionTreeClassifier(max_depth=18, max_features='sqrt'), vars, "dt", train, test,
                             seed)
    nbDefault = baseModel(BernoulliNB(), vars, "bayes", train, test, seed)
    svcDefault = baseModel(SVC(probability=True), vars, "svc", train, test, seed)
    knnDefault = baseModel(KNeighborsClassifier(), vars, "knn", train, test, seed)
    return train, test
