...
import pickle
import scipy
import chainer  #深度学习框架
import chainer.functions as F
import chainer.links as L
from chainer import optimizers #优化/调整内部参数以适应目标函数
from chainer import serializers  #处理保存/加载 Chain 模型
...
######################################################################################
# Define model
class Model(chainer.Chain): #调整模型的内部参数来完成Chain类完成
    """
    ...
    """
    def __init__(self, n_in, n_units1, n_units2, n_out):  #构造器
        super(Model, self).__init__(
            l1=L.Linear(n_in, n_units1),
            l2=L.Linear(n_units1, n_units2),
            l3=L.Linear(n_units2, n_out),
        )
    def __call__(self, x_i, x_j, t_i, t_j):  #将一个类实例当做函数调用
        s_i = self.l3(F.relu(self.l2(F.relu(self.l1(x_i)))))  #relu 激活函数
...
class RankNet(NNfuncs.NN): # NNfunsc.NN 父类
    """
    ...
    def __init__(self, resumemodelName=None, verbose=True):
        ...
        if resumemodelName is not None:
            ...
            self.model =self.loadModel(resumemodelName)
    ...
    def fit(self, fit_X, fit_y, batchsize=100, n_iter=5000, n_units1=512, n_units2=128, tv_ratio=0.95, optimizerAlgorithm="Adam", savefigName="RankNetresult.pdf", savemodelName="RankNet.model"):
